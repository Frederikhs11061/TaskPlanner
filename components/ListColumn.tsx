...
interface Props {
  list: List
  onAddCard: (listId: string, title: string, desc: string, priority: Priority, image?: string | null) => void
  ...
  return (
    ...
    >
      ...
      <div style={{ padding:'5px 9px 9px' }}>
        {isAdding ? (
          <AddCardForm
            onAdd={(title, desc, priority, image) => {
              onAddCard(list.id, title, desc, priority, image)
              setIsAdding(false)
            }}
